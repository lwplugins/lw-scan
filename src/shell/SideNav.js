 ...
 */
import LwMark from '../components/LwMark';
		...
		<aside className={ `lw-admin-sidebar ${ isOpen ? 'is-open' : '' }` }>
			<div className="lw-admin-sidebar__head">
				<a
					...
					href={ `#${ tabs[ 0 ].id }` }
					aria-label={ __( 'LW Scan home', 'lw-scan' ) }
				>
					<LwMark />
					<strong>LW Scan</strong>
				</a>
			...
			<nav
				...
				aria-label={ __( 'LW Scan sections', 'lw-scan' ) }
			>
				<ul>
					{ tabs.map( ( tab ) => {
			...
			<div className="lw-admin-sidebar__foot">
				...
				>
					...
					<span className="lw-admin-sidenav__label">
						{ __( 'Documentation', 'lw-scan' ) }
					</span>
					<Icon icon={ external } size={ 16 } />
					<span className="screen-reader-text">
						{ __( '(opens in a new tab)', 'lw-scan' ) }
					</span>
				</a>
			</div>
		</aside>
