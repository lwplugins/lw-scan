<?php
/**
 * Tests for Admin\Ajax\StatusHandler::build_payload().
 *
...
use LightweightPlugins\Scan\Admin\Ajax\StatusHandler;
use PHPUnit\Framework\TestCase;

/**
 * `build_payload()` is pure — no WP functions, no `$wpdb` access — so these
 ...
 */
final class StatusPayloadTest extends TestCase {
	...
	public function test_merges_progress_fields_into_the_top_level(): void {
		$payload = StatusHandler::build_payload( $this->sample_progress(), [], $this->sample_counts(), null );
	...
	public function test_shapes_each_feed_row_from_the_raw_findings_columns(): void {
		...
		$payload = StatusHandler::build_payload( $this->sample_progress(), [ $row ], $this->sample_counts(), null );
	...
	public function test_feed_is_capped_at_twenty_rows(): void {
		...
		$payload = StatusHandler::build_payload( $this->sample_progress(), array_fill( 0, 25, $row ), $this->sample_counts(), null );

		$this->assertCount( 20, $payload['feed'] );
	}

	public function test_counts_pass_through_unchanged(): void {
		$counts  = $this->sample_counts();
		$payload = StatusHandler::build_payload( $this->sample_progress(), [], $counts, null );

		$this->assertSame( $counts, $payload['counts'] );
	}
	...
	public function test_last_run_is_summarized_to_id_status_and_timestamps(): void {
		...
		$payload = StatusHandler::build_payload( $this->sample_progress(), [], $this->sample_counts(), $run );
